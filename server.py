...
FORBIDDEN = ("/imgs/forbidden1.jpg","/imgs/forbidden2.jpg") 
...
def handle_client_request(resource, client_socket):
    ...
    if  resource in FORBIDDEN : 
        print("--------------------------------------------------enter forbbideb ------------------------------------------------------------")
        code = "403 Forbidden"
        filename = root +ROOTwebroot +"/error403.html"
            
    if url in REDIRECTION_DICTIONARY:
        print("--------------------------------------------------enter MOVED------------------------------------------------------------")
        http_header = HTTP_VERSION+" 302 Temporarily Moved\r\nLocation:%s\r\n\r\n" %(REDIRECTION_DICTIONARY[url])# TO DO: generate proper HTTP header
        client_socket.send(http_header.encode())
        return 
    if code == "200 OK":
        filename = root +ROOTwebroot + url
        print(filename)  
    ...
    if(data == None):
        ...
        code =  "500  Not Found"
        filename =root +ROOTwebroot +"/error500.html" ;
        data = get_file_data(filename)
    ...
    if filetype == "ico" :
        ...
        http_header = HTTP_VERSION+" %s\r\nContent-Length: %s\r\nContent-Type: text/js; callablecharset=utf-8\r\n\r\n"%(code,os.path.getsize(filename)) # TO DO: generate proper HTTP header 
    elif filetype == 'html'  :
